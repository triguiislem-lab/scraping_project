...
def parse_cecmed_records(page: str, base_url: str) -> List[Dict[str, str]]:
    ...
    for idx, anchor in enumerate(anchors):
        ...
        if not product or product.lower().startswith(("descargar", "page", "siguiente", "ultima", "última")):
            continue
        pdf_anchor = None
        for next_anchor in anchors[idx + 1 : idx + 8]:
            next_label = norm(next_anchor.get("label"))
            if "DESCARGAR" in next_label or next_anchor.get("href", "").lower().endswith((".pdf", "/download")):
...
def cecmed_search(query: str, raw_dir: Path, timeout: int, resume: bool, max_pages: int = 3) -> Tuple[str, str, List[Dict[str, str]]]:
    ...
    records: List[Dict[str, str]] = []
    statuses: List[str] = []
    # Drupal exposed filters have changed names across CECMED versions. Try common
    # field names first, then fall back to a bounded page scan.
    filter_keys = [
        "field_nombre_producto_value",
        "field_nombre_del_producto_value",
        "field_nombre_principio_activo_value",
        "title",
    ]
    candidate_urls = [f"{CECMED_RCP}?{key}={urllib.parse.quote(query)}" for key in filter_keys if query]
    candidate_urls.extend(f"{CECMED_RCP}?page={page}" for page in range(max_pages))
    for url in candidate_urls:
        ...
        if query:
            qnorm = norm(query)
            page_records = [record for record in page_records if qnorm in norm(record.get("product") + " " + record.get("context"))]
        records.extend(page_records)
        if records and "?page=" not in url:
            break
    deduped: List[Dict[str, str]] = []
    seen: set[str] = set()
    for record in records:
...
def cecmed_sections(
    ...
    statuses: List[str] = []
    for query in query_candidates(row)[:2]:
...
def main() -> int:
    ...
    parser.add_argument("--replace-output", action="store_true", help="Rewrite the section output with only this run's rows instead of preserving previously recovered sections.")
    ...
    for idx, row in enumerate(queue_rows, start=1):
        ...
        for source in sources:
            ...
            if sections:
                ...
                break
        if not chosen_status:
            chosen_status = "no_relevant_source_after_country_routing"
        if chosen_sections:
    ...
    existing_section_rows_merged = 0
